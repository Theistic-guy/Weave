...
from PyQt5.QtWidgets import (
    ...
    QGraphicsPathItem, QInputDialog, QMenu, QColorDialog, QFrame, QMessageBox
)
from PyQt5.QtCore import Qt, QPointF, QRectF, QLineF, pyqtSignal, QTimer
from PyQt5.QtGui import QPainter, QPen, QBrush, QColor, QFont, QPainterPath, QCursor

from config import (
    gc, qc, new_id, SETTINGS, NODE_TYPE_COLORS, EDGE_DIRECTIONS,
...
class EdgeItem(QGraphicsPathItem):
    ...
    def _refresh_label_text(self):
        parts = []
        if self.edge_type != "relationship": parts.append(f"[{self.edge_type}]")
        if self.label: parts.append(self.label)
        self._label_item.setPlainText("  ".join(parts) if parts else "")
        self._label_item.setFont(QFont("Segoe UI", SETTINGS["ui_font_size"] - 1))
    ...
    def update_path(self):
        sp = self.source_node.scenePos(); tp = self.target_node.scenePos()
        r_s = self.source_node.radius; r_t = self.target_node.radius
        ...
        path = QPainterPath(src_pt); path.lineTo(tgt_pt)
        angle = math.atan2(dy, dx); arrow_len = 9

        def arrow_at(pt, ang):
            a1, a2 = ang + math.pi*0.82, ang - math.pi*0.82
            arr = QPainterPath(); arr.moveTo(pt)
            arr.lineTo(pt + QPointF(math.cos(a1)*arrow_len, math.sin(a1)*arrow_len))
            arr.moveTo(pt); arr.lineTo(pt + QPointF(math.cos(a2)*arrow_len, math.sin(a2)*arrow_len))
            return arr
    ...
    def paint(self, painter, option, widget=None):
        ...
        pen = QPen(qc("ACCENT") if self.isSelected() else QColor(self.color), 2.0 if self.isSelected() else 1.2, style)
        self.setPen(pen)
...
class NodeItem(QGraphicsItem):
    def __init__(self, node_id=None, label="Node", x=0, y=0, node_type="default", color=None, properties=None):
        ...
        self._inject_schema()
        self.setFlag(QGraphicsItem.ItemIsMovable); self.setFlag(QGraphicsItem.ItemIsSelectable); self.setFlag(QGraphicsItem.ItemSendsGeometryChanges)
        self.setPos(x, y); self.setZValue(1)
    ...
    def _inject_schema(self):
        ...
        for k in expected:
            if k not in self.properties: self.properties[k] = ""

    def _refresh_text(self):
        self.prepareGeometryChange()
        self._text.setPlainText(self.label)
        self._text.setFont(QFont("Segoe UI", SETTINGS["ui_font_size"], QFont.Medium))
        self._text.setDefaultTextColor(qc(self.color))
        br = self._text.boundingRect()
        self._text.setPos(self.radius + 5, -br.height() / 2)

    def boundingRect(self):
        p = 5; return QRectF(-self.radius-p, -self.radius-p, self.radius*2+p*2, self.radius*2+p*2)

    def paint(self, painter, option, widget=None):
        painter.setRenderHint(QPainter.Antialiasing)
        r = self.radius; col = qc("ACCENT") if self.isSelected() else QColor(self.color)
        painter.setBrush(QBrush(col)); painter.setPen(Qt.NoPen); painter.drawEllipse(QPointF(0, 0), r, r)
        if self.isSelected():
            painter.setBrush(Qt.NoBrush); painter.setPen(QPen(col.lighter(150), 2)); painter.drawEllipse(QPointF(0, 0), r+4, r+4)
        
        filled_props = sum(1 for v in self.properties.values() if str(v).strip() != "")
        if filled_props > 0:
            br = QRectF(r-6, -r-2, 12, 10); painter.setBrush(QBrush(QColor("#ff6584"))); painter.setPen(Qt.NoPen)
            painter.drawRoundedRect(br, 3, 3); painter.setPen(QPen(Qt.white)); painter.setFont(QFont("Segoe UI", 6, QFont.Bold))
    ...
    def add_edge(self, e):
        if e not in self.edges: self.edges.append(e); self._update_size()

    def remove_edge(self, e):
        if e in self.edges: self.edges.remove(e); self._update_size()

    def _update_size(self):
        self.prepareGeometryChange(); self.radius = 14 + len(self.edges) * 2; self._refresh_text()
        for e in self.edges: e.update_path()
        self.update()
...
class GraphScene(QGraphicsScene):
    node_selected = pyqtSignal(object); edge_selected = pyqtSignal(object); graph_changed = pyqtSignal()

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setSceneRect(-100000, -100000, 200000, 200000); self.setBackgroundBrush(QBrush(qc("BG_DARK")))
        self.nodes = {}; self.edges = {}
        self._connecting = False; self._conn_source = None; self._conn_line = None
        self.layout_timer = QTimer(); self.layout_timer.timeout.connect(self._layout_step); self.layout_active = False
    ...
    def _layout_step(self):
        forces = {nid: QPointF(0, 0) for nid in self.nodes}; nl = list(self.nodes.values())
        C_REP, C_ATT, IDEAL = 3000.0, 0.04, 150.0
        for i in range(len(nl)):
            for j in range(i+1, len(nl)):
                a, b = nl[i], nl[j]
                dx = a.x() - b.x(); dy = a.y() - b.y(); d = math.hypot(dx, dy) or 0.1
                if d > 1000: continue
        ...
        for e in self.edges.values():
            ...
            dx = b.x()-a.x(); dy = b.y()-a.y(); d = math.hypot(dx, dy) or 0.1
            f = C_ATT * (d - IDEAL); fx, fy = f*dx/d, f*dy/d
            forces[a.node_id] += QPointF(fx, fy); forces[b.node_id] -= QPointF(fx, fy)
        cap = 12.0
        for node in nl:
            if node.isSelected(): continue
            f = forces[node.node_id]; mx = max(-cap, min(cap, f.x())); my = max(-cap, min(cap, f.y()))
    ...
    def add_node(self, label="Node", x=0, y=0, node_type=None, color=None, properties=None, node_id=None):
        ...
        self.nodes[n.node_id] = n; self.addItem(n); self.graph_changed.emit()
        return n
    ...
    def add_edge(self, src, tgt, label="", edge_id=None, direction=None, edge_type=None, color=None):
        ...
        self.edges[e.edge_id] = e; self.addItem(e); src.add_edge(e); tgt.add_edge(e); self.graph_changed.emit()
        return e
    ...
    def to_dict(self):
        return {
            "types": {"nodes": NODE_TYPE_COLORS, "edges": EDGE_TYPE_COLORS, "schema": PROPERTY_SCHEMA, "settings": SETTINGS},
    ...
    def load_dict(self, data):
        ...
        if "edges" in types: 
            ...
            else: EDGE_TYPE_COLORS.update(types["edges"])
        ...
        for nd in data.get("nodes", []):
            self.add_node(label=nd["label"], x=nd["x"], y=nd["y"], node_type=nd.get("node_type"), color=nd.get("color"), properties=nd.get("properties", {}), node_id=nd["id"])
        for ed in data.get("edges", []):
            s = self.nodes.get(ed["source"]); t = self.nodes.get(ed["target"])
            if s and t: self.add_edge(s, t, label=ed.get("label",""), edge_id=ed["id"], direction=ed.get("direction","→"), edge_type=ed.get("edge_type","relationship"), color=ed.get("color"))
    ...
    def mouseReleaseEvent(self, event):
        if self._connecting:
            ...
            self.abort_connect(); return
        super().mouseReleaseEvent(event)

    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            items = self.items(event.scenePos()); clicked = None
            for i in items:
                if isinstance(i, (NodeItem, EdgeItem)): clicked = i; break
                elif isinstance(i, QGraphicsTextItem):
                    parent = i.parentItem()
                    if isinstance(parent, (NodeItem, EdgeItem)): clicked = parent; break
            if isinstance(clicked, NodeItem): self.node_selected.emit(clicked); self.edge_selected.emit(None)
            elif isinstance(clicked, EdgeItem): self.edge_selected.emit(clicked); self.node_selected.emit(None)
            else: self.node_selected.emit(None); self.edge_selected.emit(None)
...
class CanvasView(QGraphicsView):
    ...
    def __init__(self, scene):
        ...
        self.setDragMode(QGraphicsView.RubberBandDrag); self.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff); self.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.setFrameShape(QFrame.NoFrame)
        self._draw_grid = True; self._panning = False; self._pan_start = None; self._space_held = False; self._scale = 1.0

    def drawBackground(self, painter, rect):
        super().drawBackground(painter, rect); if not self._draw_grid: return
        painter.setRenderHint(QPainter.Antialiasing, False)
        grid = 40; is_dark = config.CURRENT_THEME == "dark"
        pen_minor = QPen(QColor(128, 128, 128, 12 if is_dark else 10), 0); pen_major = QPen(QColor(128, 128, 128, 25 if is_dark else 20), 0)
        ...
        for x in range(l, r, grid):
            painter.setPen(pen_major if x % (grid*5) == 0 else pen_minor); painter.drawLine(x, int(rect.top()), x, int(rect.bottom()))
        for y in range(t, b, grid):
            painter.setPen(pen_major if y % (grid*5) == 0 else pen_minor); painter.drawLine(int(rect.left()), y, int(rect.right()), y)
    ...
    def _set_zoom(self, z):
        ...
        factor = z / self._scale; self._scale = z; self.scale(factor, factor); self.zoom_changed.emit(self._scale)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Space and not event.isAutoRepeat():
            ...
            self.scene().abort_connect(); self._space_held = False; self.setDragMode(QGraphicsView.RubberBandDrag); self.setCursor(Qt.ArrowCursor)
        super().keyPressEvent(event)

    def keyReleaseEvent(self, event):
        if event.key() == Qt.Key_Space and not event.isAutoRepeat():
            self._space_held = False; self._panning = False; self._pan_start = None; self.setDragMode(QGraphicsView.RubberBandDrag); self.setCursor(Qt.ArrowCursor)
    ...
    def mouseReleaseEvent(self, event):
        if self._panning and (event.button() == Qt.MiddleButton or event.button() == Qt.LeftButton):
            self._panning = False; self._pan_start = None; self.setCursor(Qt.OpenHandCursor if self._space_held else Qt.ArrowCursor)
            if not self._space_held: self.setDragMode(QGraphicsView.RubberBandDrag)
            return
        super().mouseReleaseEvent(event)

    def contextMenuEvent(self, event):
        scene_pos = self.mapToScene(event.pos()); items = self.scene().items(scene_pos)
        ...
        if node:
            a_conn = menu.addAction("🔗  Connect from here"); a_edit = menu.addAction("✏️  Rename"); a_col = menu.addAction("🎨  Change colour")
            menu.addSeparator(); a_copy = menu.addAction("📋  Copy  (Ctrl+C)"); menu.addSeparator(); a_del = menu.addAction("🗑️  Delete node")
            ...
            a_lbl = menu.addAction("🏷️  Edit label"); a_col = menu.addAction("🎨  Change colour")
    ...
    def _paste(self, at=None):
        ...
        for nd in self._clipboard:
            ...
            self.scene().add_node(label=nd["label"], x=pos.x(), y=pos.y(), node_type=nd.get("node_type"),
                color=nd.get("color"), properties=copy.deepcopy(nd.get("properties", {})), node_id=new_id())
